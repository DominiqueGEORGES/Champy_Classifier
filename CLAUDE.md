...
15. **Ne JAMAIS push sur main**. Tous les push vont sur `dev-dominique` uniquement (`git push origin dev-dominique`). Un merge vers `main` se fait par Pull Request, validée par l'équipe. Le repo DagsHub `LoicFocraud/Champy_Classifier` est le repo partagé : on y lit (DVC pull, MLflow), on n'écrit que sur notre branche.
